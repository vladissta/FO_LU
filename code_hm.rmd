...
colnames(counts_hm) = gsub('MI39','',colnames(c_tpm_hm_centered))
```
...
```{r}
superheat::superheat(as.matrix(c_tpm_hm_centered), bottom.label.text.size = 4, 
                     bottom.label.text.angle = 50,
                     left.label.text.size = 4,
                     title = 'TPM of SIX genes in each sample')


pheatmap::pheatmap(countz, cluster_rows = F, cluster_cols = F,
                   color = c('white', colorRampPalette(c('lightblue','firebrick'))(10000)), 
                   display_numbers = F, number_color = 'black', main = 'Counts of SIX genes in each sample')
```
